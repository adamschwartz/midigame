$(function(){
    game.first_piece = {'note': {'pitch': 100, 'velocity': 72}, 'rect': [-1500, 340, 1450, 20]};
    game.map = {};

    game.map.ground = game.midi.mary;
    ...
    game.update_dude_position = function() {
        ...
        if (Math.abs(vy) < 0.02) { //TODO - tweak this
            vy = 0;
        }
        ...
        game.dude.dx = dx;
        game.dude.dy = dy;
        game.helpers.dude_collision_new();
    ...
    game.draw_ground = function() {
        ...
        for (i in game.map.ground) {
            ...
            if (g.rect[0] < 20000) { //TODO - dynamically load the part of the ground we need
                ...
                //c = g.note.color ? g.note.color : 'rgba(100, 200, ' + (30 * (g.note.pitch % 12)) + ', 1)'; //green
                c = g.note.color ? g.note.color : 'rgba(' + (30 * (g.note.pitch % 12)) + ', 100, 200, 1)'; //purple
        ...
        if (false) {
            var j;
            for (i in game.collision.mary) {
                for (j in game.collision.mary[i]) {
                    if (game.collision.mary[i][j] === 1) {
                        g = {
                            note: {
                                pitch: game.collision.lowest.mary + i
                            },
                            rect: [
                                j * game.collision.scaleX.mary,
                                ((13 + (game.collision.mary.length - i)) * 20),
                                20,
                                20
                            ]
                        };
    ...
    game.helpers.dude_collision_new = function() {
        var oldX = game.dude.css.left,
            oldY = game.dude.css.top,
            newX = oldX + game.dude.dx,
            newY = oldY + game.dude.dy,
            blockedX, blockedY
        ;
        ...
        if (oldY != newY) { // moving vertically
            if (newY > oldY) { // moving down
                ...
                collision = game.helpers.is_blocking(newX + space, newY + 20);
                if (collision && !game.helpers.is_blocking(newX + space, newY)) {
                    newY -= collision[1];
                    game.dude.on_ground = true;
                    blockedY = true;
                }

                // lower right collision
                collision = game.helpers.is_blocking(newX + 20-space, newY + 20);
                if (collision && !game.helpers.is_blocking(newX + 20-space, newY)) {
                    newY -= collision[1];
                    game.dude.on_ground = true;
                    blockedY = true;
                }
            ...
            // moving up
                ...
                collision = game.helpers.is_blocking(newX + space, newY);
                if (collision && !game.helpers.is_blocking(newX + space, newY + 20)) {
                    newY += (20 - collision[1]);
                    blockedY = true;
                }

                // upper right collision
                collision = game.helpers.is_blocking(newX + 1 - space, newY);
                if (collision && !game.helpers.is_blocking(newX + 1 - space, newY + 20)) {
                    newY += (20 - collision[1]);
                    //xAdjust = 1;
                    blockedY = true;
                }
        ...
        if (oldX != newX) {
            ...
            if (newX > oldX) {
                ...
                // lower right collision
                collision = game.helpers.is_blocking(newX + 20, newY + 20-space);
                if (collision) {
                    newX -= collision[0];
                    blockedX = true;
                }
                ...
                // upper right collision
                collision = game.helpers.is_blocking(newX + 20, newY);
                if (collision) {
                    newX -= collision[0];
                    blockedX = true;
                }
            ...
            // moving left
                ...
                // lower left collision
                collision = game.helpers.is_blocking(newX, newY + 20-space);
                if (collision) {
                    newX += (20 - collision[0]);
                    blockedX = true;
                }
                ...
                // upper left collision
                collision = game.helpers.is_blocking(newX, newY);
                if (collision) {
                    newX += (20 - collision[0]);
                    blockedX = true;
                }
        ...
        game.dude.dx = newX - oldX;
        game.dude.dx = newY - oldY;

        //return [newX,newY,xAdjust];
    };

    // check if a tile is blocking
    game.helpers.is_blocking = function(px, py) {
        var x = ((px - game.ground.left) / game.collision.scaleX.mary);
        var y = (13 - (py / 20) + game.collision.mary.length);
        ...
        if (game.collision.mary[ty] && game.collision.mary[ty].length && game.collision.mary[ty][tx] === 1) {
            console.log('Colllision!', 'px', px, 'py', py, 'x', x, 'tx', tx, 'y', y, 'ty', ty);
            //return [x - tx, y - ty];
            return [(x - tx) * (px / x), (y - ty) * (py / y)];
        }

        //typeof game.collision.mary[ty][tx] == 'undefined'
